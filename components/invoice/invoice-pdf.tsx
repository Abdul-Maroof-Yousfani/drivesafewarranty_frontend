...
export const InvoicePDF = ({ settings, data }: InvoicePDFProps) => {
    ...
    if (settings.logoPosition === "center") {
        logoPositionStyle.alignSelf = "center";
    } else if (settings.logoPosition === "right") {
        logoPositionStyle.alignSelf = "flex-end";
    } else {
        logoPositionStyle.alignSelf = "flex-start";
    }
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.container}>
                    ...
                    <View style={styles.headerSection}>
                        <View style={styles.companyInfo}>
                            ...
                            <Text style={[styles.companyName, logoPositionStyle]}>{isSettlement ? "Drive Safe" : settings.companyName}</Text>
                            <Text style={[styles.companyAddress, logoPositionStyle]}>{isSettlement ? "Your Partner in Road Safety" : settings.companyAddress}</Text>
                        </View>

                        <View style={styles.invoiceInfo}>
                            <Text style={styles.invoiceTitle}>{isSettlement ? "SETTLEMENT" : settings.headerText || "INVOICE"}</Text>
                            <Text style={styles.invoiceDetail}><Text style={styles.boldText}>Invoice #:</Text> {data.invoiceNumber}</Text>
                            <Text style={styles.invoiceDetail}><Text style={styles.boldText}>Date:</Text> {data.date}</Text>
                            {data.dueDate && (
                                <Text style={styles.invoiceDetail}><Text style={styles.boldText}>Due Date:</Text> {data.dueDate}</Text>
                            )}
                        </View>
                    </View>
                    ...
                    <View style={styles.billToCard}>
                        <View style={styles.billToContainer}>
                            <Text style={styles.billToLabel}>{isSettlement ? "Dealer (Bill To):" : settings.billToTitle || "Bill To:"}</Text>
                            <Text style={styles.billToName}>{data.billToName}</Text>
                            {data.billToAddress && <Text style={styles.billToAddress}>{data.billToAddress}</Text>}
                            {data.billToEmail && <Text style={styles.billToAddress}>{data.billToEmail}</Text>}
                        </View>
                        {data.duration && (
                            <View style={styles.durationBadge}>
                                <Text style={styles.durationText}>Plan Duration: {data.duration}</Text>
                            </View>
                        )}
                    </View>
                                ...
                                <View style={{ flex: 3 }}>
                                    ...
                                    {isSettlement && item.cost && (
                                        <Text style={styles.dealerDetail}>
                                            Customer Price: {formatCurrency(item.amount)} | Margin: {formatCurrency(item.amount - item.cost)}
                                        </Text>
                                    )}
                                </View>
                                <Text style={styles.cellQty}>{item.quantity}</Text>
                                <Text style={styles.cellAmount}>{formatCurrency(isSettlement && item.cost !== undefined ? item.cost : item.amount)}</Text>
                            </View>
                        ))}
                    </View>
                ...
                <View style={styles.footerSection}>
                    ...
                    <View style={styles.footer}>
                        <Text>{isSettlement ? "Official Drive Safe Settlement Statement" : (settings.footerText || "Thank you for your business!")}</Text>
                    </View>
                </View>
            </Page>
        </Document>

  ...
  try {
    ...
    if (!res.ok) {
      const errorData = await res
        .json()
        .catch(() => ({ message: "Failed to fetch employee statuses" }));
      return {
        status: false,
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to fetch employee statuses. Please check your connection.",
    };
  }
}

// Get employee status by id
export async function getEmployeeStatusById(
  id: string
): Promise<{ status: boolean; data?: EmployeeStatus; message?: string }> {
  try {
    ...
    if (!res.ok) {
      const errorData = await res
        .json()
        .catch(() => ({ message: "Failed to fetch employee status" }));
      return {
        status: false,
        message: errorData.message || `HTTP error! status: ${res.status}`,
      };
...
export async function createEmployeeStatus(data: {
  status: string;
  statusType?: string;
}): Promise<{ status: boolean; data?: EmployeeStatus; message?: string }> {
  try {
    ...
    if (!res.ok) {
      const errorData = await res
        .json()
        .catch(() => ({ message: "Failed to create employee status" }));
      return {
        status: false,
        message: errorData.message || `HTTP error! status: ${res.status}`,
      };
...
export async function createEmployeeStatusesBulk(
  ...
  try {
    ...
    if (!res.ok) {
      const errorData = await res
        .json()
        .catch(() => ({ message: "Failed to create employee statuses" }));
      return {
        status: false,
        message: errorData.message || `HTTP error! status: ${res.status}`,
      };
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to create employee statuses",
    };
  }
}
...
export async function updateEmployeeStatus(
  ...
  try {
    ...
    if (!res.ok) {
      const errorData = await res
        .json()
        .catch(() => ({ message: "Failed to update employee status" }));
      return {
        status: false,
        message: errorData.message || `HTTP error! status: ${res.status}`,
      };
...
export async function deleteEmployeeStatus(
  ...
  try {
    ...
    if (!res.ok) {
      const errorData = await res
        .json()
        .catch(() => ({ message: "Failed to delete employee status" }));
      return {
        status: false,
        message: errorData.message || `HTTP error! status: ${res.status}`,
      };
...
export async function updateEmployeeStatuses(
  ...
  try {
    ...
    if (failed.length > 0) {
      return {
        status: false,
        message: `${failed.length} employee status(es) failed to update`,
      };
    }

    return {
      status: true,
      message: `${results.length} employee status(es) updated successfully`,
    };
  } catch (error) {
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to update employee statuses",
    };
  }
}

// Delete employee statuses bulk
export async function deleteEmployeeStatuses(
  ids: string[]
): Promise<{ status: boolean; message?: string }> {
  try {
    const results = await Promise.all(
    ...
    if (failed.length > 0) {
      return {
        status: false,
        message: `${failed.length} employee status(es) failed to delete`,
      };
    }

    return {
      status: true,
      message: `${results.length} employee status(es) deleted successfully`,
    };
  } catch (error) {
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to delete employee statuses",
    };
  }
}

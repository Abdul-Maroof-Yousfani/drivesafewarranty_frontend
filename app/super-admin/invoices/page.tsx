...
import { getAllInvoicesAction, Invoice, updateInvoiceAction } from "@/lib/actions/invoices";
...
import { FileText, Loader2, CheckCircle, Search, Wallet, Banknote, Clock } from "lucide-react";
...
export default function SuperAdminInvoicesPage() {
    ...
    const paidAmount = paidInvoicesList.reduce((sum, inv) => sum + Number(inv.amount), 0);
    
    const pendingInvoicesList = invoices.filter(inv => inv.status === 'pending');
    const pendingCount = pendingInvoicesList.length;
    const pendingAmount = pendingInvoicesList.reduce((sum, inv) => sum + Number(inv.amount), 0);
    ...
    const loadInvoices = async (dealerId?: string) => {
        ...
        if (res.status && res.data) {
            ...
            const settlementInvoices = res.data.invoices.filter(inv => !!inv.dealerId);
    ...
    const handleMarkAsPaid = async (id: string) => {
        try {
            const res = await updateInvoiceAction(id, { status: "paid" });
            if (res.status) {
                        ...
                        <CardContent>
                            {!selectedDealerId ? (
                                <div className="text-center text-muted-foreground border-2 border-dashed rounded-lg">
                                    ...
                                    <p>Please select a dealer from the dropdown above to view their settlement details.</p>
                                </div>
                            ) : (
                                        ...
                                        <Card className="bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800">
                                            <CardContent className="pt-3 flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <h4 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{formatCurrency(totalAmount)}</h4>
                                                    <p className="text-sm text-slate-500 dark:text-slate-400">{totalInvoices} Invoices Total</p>
                                                </div>
                                        ...
                                        <Card className="bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-900/50">
                                            <CardContent className="pt-3 flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <h4 className="text-2xl font-bold text-emerald-800 dark:text-emerald-100">{formatCurrency(paidAmount)}</h4>
                                                    <p className="text-sm text-emerald-600 dark:text-emerald-400">{paidCount} Invoices Paid</p>
                                                </div>
                                        ...
                                        <Card className="bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-900/50">
                                            <CardContent className="pt-3 flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <h4 className="text-2xl font-bold text-amber-800 dark:text-amber-100">{formatCurrency(pendingAmount)}</h4>
                                                    <p className="text-sm text-amber-600 dark:text-amber-400">{pendingCount} Invoices Due</p>
                                                </div>
                                                    ...
                                                    {invoices.map((inv: Invoice) => {
                                                        ...
                                                        return (
                                                            <TableRow key={`inv-${inv.id}`}>
                                                                ...
                                                                <TableCell>
                                                                    <div className="flex flex-col">
                                                                        <span className="font-medium">{inv.dealer?.businessNameLegal}</span>
                                                                        <span className="text-xs text-muted-foreground">{inv.dealer?.email}</span>
                                                                    </div>
                                                                </TableCell>
                                                                <TableCell>
                                                                    {inv.warrantySale?.warrantyPackage?.name || "N/A"}
                                                                </TableCell>

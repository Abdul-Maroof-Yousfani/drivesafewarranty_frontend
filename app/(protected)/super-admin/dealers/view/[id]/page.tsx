    ...
    <div className="space-y-4 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 py-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold tracking-tight text-foreground break-all">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { 
            label: "Customers", 
            value: dealer.totalCustomers || 0, 
            icon: User, 
            color: "primary",
            href: `/super-admin/dealers/${dealer.id}/customers`
          },
          { 
            label: "Total Packages Sold", 
            value: dealer.totalWarranties || 0, 
            icon: BadgeCheck, 
            color: "blue",
            href: `/super-admin/dealers/${dealer.id}/warranties`
          },
          ...
          const CardComponent = (
            <Card className={cn(
              "shadow-none border-muted p-2 bg-card/50 h-full",
              stat.href && "hover:bg-muted/50 cursor-pointer transition-colors group"
            )}>
              <CardContent className="p-3 flex flex-col gap-0.5">
                <p className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider group-hover:text-primary transition-colors">
                  {stat.label}
                </p>
                <div className="flex items-center justify-between">
                  <span className="text-base font-bold">{stat.value}</span>
                  <stat.icon className={cn("h-8 w-8 opacity-40 transition-transform group-hover:scale-110", `text-${stat.color}-500`)} />
                </div>
              </CardContent>
            </Card>
          );
          ...
          return stat.href ? (
            <Link key={idx} href={stat.href} className="block h-full">
              {CardComponent}
            </Link>
          ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="p-2 border-b md:border-r border-muted/50 space-y-3">
                  <div>
                    ...
                    <p className="font-semibold text-xs break-all">{dealer.businessNameLegal || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-muted-foreground uppercase">Trading Name</label>
                    <p className="text-xs break-all">{dealer.businessNameTrading || dealer.businessNameLegal}</p>
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-muted-foreground uppercase">Business Address</label>
                    <p className="text-xs leading-relaxed text-muted-foreground break-all">{dealer.businessAddress || "N/A"}</p>
                  </div>
                </div>
                ...
                <div className="p-2 border-b border-muted/50 space-y-3">
                  <div>
                    ...
                    <p className="font-semibold text-xs break-all">{dealer.contactPersonName || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-muted-foreground uppercase">Email Address</label>
                    <p className="text-xs break-all">{dealer.email || "N/A"}</p>
                  </div>
                  <div>
                ...
                <div className="p-3 md:col-span-2 bg-muted/5 flex flex-wrap gap-x-8 gap-y-2">
                   <div>
                      ...
                      <p className="text-[10px] font-mono bg-background px-1.5 py-0.5 rounded border inline-block break-all">{dealer.dealerLicenseNumber || "N/A"}</p>
                   </div>
                   <div>
                      <label className="text-[9px] font-bold text-muted-foreground uppercase block mb-0.5">Registration Number</label>
                      <p className="text-[10px] font-mono bg-background px-1.5 py-0.5 rounded border inline-block break-all">{dealer.businessRegistrationNumber || "N/A"}</p>
                   </div>
                </div>
              </div>
            </CardContent>
        ...
        <div className="space-y-4">
            ...
            <CardContent>
               <div className="space-y-3">
                 <div>
                   ...
                   <p className="text-xs font-medium break-all">{dealer.bankDetails?.bankName || "N/A"}</p>
                 </div>
                 <div>
                   <label className="text-[9px] font-bold text-muted-foreground uppercase">Account Holder</label>
                   <p className="text-xs font-medium break-all">{dealer.bankDetails?.accountHolderName || "N/A"}</p>
                 </div>
                 <div>
                   <label className="text-[9px] font-bold text-muted-foreground uppercase">Account Number</label>
                   <p className="text-xs font-mono tracking-tighter break-all">{dealer.bankDetails?.accountNumber || "N/A"}</p>
                 </div>
                 <div>
                   <label className="text-[9px] font-bold text-muted-foreground uppercase">Routing / Sort Code</label>
                   <p className="text-xs font-mono break-all">{dealer.bankDetails?.routingNumber || "N/A"}</p>
                 </div>
               </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-none border-muted overflow-hidden">
            ...
            <CardContent className=" space-y-3">
               <div>
                  <p className="font-bold text-xs break-all">{dealer.authorizedSignatory?.name || "N/A"}</p>
                  <p className="text-[12px] text-muted-foreground break-all">{dealer.authorizedSignatory?.title || "Legal representative"}</p>
               </div>
               <div className="space-y-1 pt-2 border-t">
                  <div className="flex items-center gap-1.5 text-[12px] truncate">
                    <Mail className="h-3 w-3 text-muted-foreground shrink-0" />
                    <span> <span className="font-bold">Email:</span> <span className="break-all">{dealer.authorizedSignatory?.email || "N/A"}</span></span>
                  </div>
